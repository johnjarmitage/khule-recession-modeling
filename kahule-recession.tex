...
\subsection{Discharge and recession estimation}
...
We subsequently note that the data show a separation between trends for early and late recession, and split the binned data to run two linear regressions. Following the method outlined in \cite{tashie-etal-2020}, we search for the knickpoint in the recession that provides the highest early recession $b$ parameter, and assume that htis marks the transition from early ot late recession. Linear regressions are run using a bootstrap method to asses the range of potential $b$ for both early and late recession.
...
The 5 minute stage height data is converted to a discharge using the Manning-Strickler and Chezy model and subsequently the recession is analysed for the three post-monsoon seasons from 2016 to 2018. The two models to convert stage height to discharge give very similar trends for the recession analysis (Figure \ref{fg:recession}). The discharge magnitude is slightly higher with the Chézy model, however this is due to the choice of the roughness coefficient. The recessions are split into an early and late recession following the method outlined in \cite{tashie-etal-2020}. From bootstrap re-sampling we find that the early recession $b$ is within the range $5.6 < b < 12.8$, with the two models overlapping in range (Figure \ref{fg:recession}). For the late recession the range is $2.4 < b < 3.3$ with a significant overlap in the two ranges (Figure \ref{fg:recession}).
...
\begin{figure}
\includegraphics[width=10cm]{./figures/figure06.png}
\caption{Comparison of early and late recession values from USGS observations \citep{tashie-etal-2020} and the potential range in $b$ for the Kahule Khola calculated form the bootstrap method (Figure \ref{fg:recession}).}
\label{fg:compare}
\end{figure}
